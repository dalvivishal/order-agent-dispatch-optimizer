...
class Order(Base):
    ...
    status = Column(Enum('pending', 'allocated', 'dispatched', 'delivered', 'postponed'), default='pending')
    created_at = Column(DateTime, default=datetime.utcnow)
...
class AllocationEngine:
    ...
    def allocate_orders(cls, session):
        ...
        agents = session.query(Agent).filter(Agent.is_active == True, Agent.checked_in == True).all()
        ...
        for warehouse_id in agents_by_warehouse.keys():
            ...
            warehouse_result = cls._allocate_warehouse_orders(warehouse_agents, warehouse_orders)
    ...
    def _allocate_warehouse_orders(cls, agents, orders):
        ...
        sorted_orders = sorted(orders, key=lambda x: priority_order[x.priority], reverse=True)
        ...
        for order in sorted_orders:
            ...
            if best_agent:
                ...
                total_time = best_agent['total_time'] + travel_time + order.estimated_time
                total_distance = best_agent['total_distance'] + distance
                
                # Check constraints
                if (total_time <= cls.MAX_WORKING_HOURS * 60 and 
                    total_distance <= cls.MAX_DISTANCE_KM):
                    
                    best_agent['orders'].append(order.id)
                    best_agent['total_time'] = total_time
                    best_agent['total_distance'] = total_distance
                    best_agent['estimated_pay'] = calculate_pay(len(best_agent['orders']))
        ...
        total_cost = sum(allocation['estimated_pay'] for allocation in agent_allocations if allocation['orders'])
    ...
    def _save_allocations(cls, session, result):
        ...
        for order_id in result['allocated_orders']:
            session.query(Order).filter(Order.id == order_id).update({'status': 'allocated'})
        
        for order_id in result['postponed_orders']:
            session.query(Order).filter(Order.id == order_id).update({'status': 'postponed'})
